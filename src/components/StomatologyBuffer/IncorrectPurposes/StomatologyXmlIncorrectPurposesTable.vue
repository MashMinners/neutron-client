<template>
  <div v-if="records.length !==0">
    <prime-data-table editMode="cell" class="editable-cells-table p-datatable-sm"  :value="records"
      ...
      <prime-column field="stom_xml_hm_zsl_date_z_1" header="Дата начала лечения (XML)"></prime-column>
      <prime-column field="stom_xml_hm_zsl_date_z_2" header="Дата окончания лечения (XML)"></prime-column>
      <prime-column header="Врач">
        <template #body="slotProps">
          {{getDoctor(slotProps.data.stom_xml_hm_zsl_sl_iddokt)}}
        </template>
      </prime-column>
      <prime-column field="stom_xml_hm_zsl_sl_p_cel" header="Цель посещения" :sortable="true"></prime-column>
      <prime-column field="stom_xml_hm_zsl_sl_usl_code_usl" header="Услуга"></prime-column>
    </prime-data-table>
  </div>
  <div v-else>
...
<script>
export default {
  name: "StomatologyXmlIncorrectPurposesTable",
  props: {
    records: {
      type: Array,
